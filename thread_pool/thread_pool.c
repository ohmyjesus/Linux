...
struct threadpool_t {
	...
	pthread_mutex_t thread_counter;	//记录忙状态下的线程个数的锁 -- busy_thr_num
	...
	int queue_size;			//task_queue对中实际任务数
...
threadpool_t* threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size){
	...
	do{
		...
		if(pthread_mutex_init(&(pool->lock), NULL) != 0 
			...
			printf("init the lock or conf fail");
			break;

		//启动 min_thr_num 个 work thread
		for( i = 0; i < min_thr_num; i++){
			pthread_create(&(pool->threads[i]), NULL, threadpool_thread, (void*)pool);//当一个线程被创建时，就执行回调函数
	...
	threadpool_free(poll);	//前面代码调用失败时，释放poll存储空间
...
void* threadpool_thread(void* threadpool){
	...
	while(true){
		...
		while((pool->queue_size == 0) && (!pool->shutdown)){
			...
			//清除指定数目 的空闲线程，如果要结束的线程个数大于0, 结束线程
...
void* adjust_thread(void* threadpool)
{
	...
	while(!pool->shutdown){
		...
		//创建新线程的算法 ： 任务数大于最小线程数，且存活的线程数少于最大线程个数时，如30>=10 && 40<100
		...
		// 销毁多余的空闲线程，算法： 忙线程X2小于存活的线程数 且 存活的线程数大于最小线程数
...
int is_thread_alive(pthread_t tid){
	int kill_rc = pthread_kill(tid, 0); 	//发0信号，测试是否有线程存活
	if(kill_rc = ESRCH)
		return false;
...
int main(){
	...
	threadpool_t* thp = thread_create(3,100,100);//创建线程池，池里最少3个线程，最多100个线程，队列最大100个。
	printf("pool inited");
	...
	for(i = 0; i < 20; i++){
		num[i] = i;
		printf("add task %d\n", i);
		threadpool_add(thp, process, (void*)&num[i]); //向线程池里面添加任务
	}
	sleep(10);						//等待子线程完成任务
